...
class HRFSolver:
    ...
    def rhs(self, h, u, v) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self.mode == "dw":
            ...
            if hasattr(xp, 'is_tensor') and xp.is_tensor(eta):  # PyTorch
                ...
                deta_dx = (xp.roll(eta, -1, axis=0) - xp.roll(eta, 1, axis=0)) / (2.0 * grid.dx)
                deta_dy = (xp.roll(eta, -1, axis=1) - xp.roll(eta, 1, axis=1)) / (2.0 * grid.dy)
    ...
    def apply_structures(self, dt: float):
        ...
        if self.overflow_mask is not None and self.crest_elev is not None:
            ...
            if float(self.xp.max(Hx)) > 0.0:
                ...
                if hasattr(self.xp, 'is_tensor') and self.xp.is_tensor(q_e):  # PyTorch
                    ...
                    q_w = self.xp.roll(q_e, 1, axis=0)
                    # South/north
                    q_s = Cd * W_s * (Hx ** 1.5) * math.sqrt(2 * g)
                    q_n = self.xp.roll(q_s, 1, axis=1)
                    # Only from overflow cells to non-overflow neighbors
                    nei_e = 1.0 - self.xp.roll(mask, -1, axis=0)
                    nei_w = 1.0 - self.xp.roll(mask,  1, axis=0)
                    nei_s = 1.0 - self.xp.roll(mask, -1, axis=1)
                    nei_n = 1.0 - self.xp.roll(mask,  1, axis=1)
                    q_e = q_e * mask * nei_e
    ...
    def choose_dt(self) -> float:
        ...
        if self.mode in ("dw", "dw_fv"):
            ...
            if hasattr(xp, 'is_tensor') and xp.is_tensor(eta):  # PyTorch
                eta_e = xp.roll(eta, -1, dims=0)
                eta_w = xp.roll(eta, 1, dims=0)
                eta_n = xp.roll(eta, 1, dims=1)
                eta_s = xp.roll(eta, -1, dims=1)
            else:  # NumPy
                eta_e = xp.roll(eta, -1, axis=0)
                eta_w = xp.roll(eta, 1, axis=0)
                eta_n = xp.roll(eta, 1, axis=1)
                eta_s = xp.roll(eta, -1, axis=1)
            ...
            if hasattr(xp, 'is_tensor') and xp.is_tensor(H):  # PyTorch
                H_e = xp.minimum(H, xp.roll(H, -1, dims=0))
                H_w = xp.minimum(H, xp.roll(H, 1, dims=0))
                H_n = xp.minimum(H, xp.roll(H, 1, dims=1))
                H_s = xp.minimum(H, xp.roll(H, -1, dims=1))
            else:  # NumPy
                H_e = xp.minimum(H, xp.roll(H, -1, axis=0))
                H_w = xp.minimum(H, xp.roll(H, 1, axis=0))
                H_n = xp.minimum(H, xp.roll(H, 1, axis=1))
                H_s = xp.minimum(H, xp.roll(H, -1, axis=1))
        ...
        while t < t_end:
            ...
            if output_every > 0.0 and t >= next_out:
                if verbose:
                    ...
                    if hasattr(xp, 'is_tensor') and xp.is_tensor(eta):  # PyTorch
                        deta_dx = (xp.roll(eta, -1, dims=0) - xp.roll(eta, 1, dims=0)) / (2.0 * self.grid.dx)
                        deta_dy = (xp.roll(eta, -1, dims=1) - xp.roll(eta, 1, dims=1)) / (2.0 * self.grid.dy)
                    else:  # NumPy
                        deta_dx = (xp.roll(eta, -1, axis=0) - xp.roll(eta, 1, axis=0)) / (2.0 * self.grid.dx)
                        deta_dy = (xp.roll(eta, -1, axis=1) - xp.roll(eta, 1, axis=1)) / (2.0 * self.grid.dy)
