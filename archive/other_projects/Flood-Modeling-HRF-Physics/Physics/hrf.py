...
class Grid:
    ...
    def __post_init__(self):
        ...
        try:
            ...
            x = self.xp.arange(self.nx, dtype=float) * self.dx + (0.5 * self.dx)
        if self.ny > 1:
            try:
                y = self.xp.arange(self.ny, device=self.device, dtype=self.xp.float32) * self.dy + (0.5 * self.dy)
            except TypeError:
                y = self.xp.arange(self.ny, dtype=float) * self.dy + (0.5 * self.dy)
        else:
            try:
                y = self.xp.array([0.5 * self.dy], device=self.device, dtype=self.xp.float32)
            except TypeError:
                y = self.xp.array([0.5 * self.dy], dtype=float)
        
        self.X, self.Y = self.xp.meshgrid(x, y, indexing="ij")
        
        # Wavenumbers (periodic domain)
        kx = 2 * np.pi * self.xp.fft.fftfreq(self.nx, d=self.dx)
        if hasattr(kx, 'to'):
            kx = kx.to(self.device)
        if self.ny > 1:
            ky = 2 * np.pi * self.xp.fft.fftfreq(self.ny, d=self.dy)
            if hasattr(ky, 'to'):
                ky = ky.to(self.device)
        else:
            try:
                ky = self.xp.array([0.0], device=self.device)
            except TypeError:
                ky = self.xp.array([0.0])
...
class HRFSolver:
    ...
    def initialize(self, h0: np.ndarray, u0: np.ndarray, v0: np.ndarray):
        try:
            self.h = self.xp.asarray(h0, device=self.device, dtype=self.xp.float32)
            self.u = self.xp.asarray(u0, device=self.device, dtype=self.xp.float32)
            self.v = self.xp.asarray(v0, device=self.device, dtype=self.xp.float32)
        except TypeError:
            self.h = self.xp.asarray(h0, dtype=self.xp.float32)
            self.u = self.xp.asarray(u0, dtype=self.xp.float32)
            self.v = self.xp.asarray(v0, dtype=self.xp.float32)
        self.time = 0.0
        self.mass0 = self.total_mass()
        # Mass budget tracking
        self.rain_total = 0.0
        self.infil_total = 0.0
        self.boundary_flux_total = 0.0
        self.source_total = 0.0  # internal sources (e.g., pump outfall injection)
        self.pond_storage_total = 0.0  # volume stored in ponds/basins (ledger)
        # Optional per-cell storage uptake rate (m/s) for engineered storage
        try:
            self.pond_storage_rate = self.xp.zeros((self.grid.nx, self.grid.ny), dtype=self.xp.float32)
        except Exception:
            self.pond_storage_rate = np.zeros((self.grid.nx, self.grid.ny), dtype=np.float32)
        self.initialize_spu()
    ...
    def set_forcing(self, bed: Optional[np.ndarray] = None,
        ...
        if bed is not None:
            try:
                self.bed = self.xp.asarray(bed, device=self.device, dtype=self.xp.float32)
            except TypeError:
                self.bed = self.xp.asarray(bed, dtype=self.xp.float32)
        
        # Helper to check for array types in a backend-agnostic way
        def is_array(x):
            # Be robust to NumPy arrays, masked arrays, memmaps, and array-likes with shape
            if self.xp is np:
                try:
                    import numpy as _np
                    return isinstance(x, _np.ndarray) or hasattr(x, 'shape')
                except Exception:
                    return hasattr(x, 'shape')
            else:  # torch backend
                try:
                    return self.xp.is_tensor(x) or hasattr(x, 'shape')
                except Exception:
                    return hasattr(x, 'shape')
        ...
        if rain_rate is not None:
            if is_array(rain_rate):
                try:
                    self.rain_rate = self.xp.asarray(rain_rate, device=self.device, dtype=self.xp.float32)
                except TypeError:
                    self.rain_rate = self.xp.asarray(rain_rate, dtype=self.xp.float32)
            else:
                try:
                    self.rain_rate = self.xp.full((self.grid.nx, self.grid.ny), float(rain_rate), device=self.device, dtype=self.xp.float32)
                except TypeError:
        ...
        if infil_rate is not None:
            if is_array(infil_rate):
                try:
                    self.infil_rate = self.xp.asarray(infil_rate, device=self.device, dtype=self.xp.float32)
                except TypeError:
                    self.infil_rate = self.xp.asarray(infil_rate, dtype=self.xp.float32)
            else:
                try:
                    self.infil_rate = self.xp.full((self.grid.nx, self.grid.ny), float(infil_rate), device=self.device, dtype=self.xp.float32)
                except TypeError:
                    self.infil_rate = self.xp.full((self.grid.nx, self.grid.ny), float(infil_rate), dtype=self.xp.float32)
        # Optional generic source (e.g., pump discharge routed to outfall cells)
        if source_rate is not None:
            if is_array(source_rate):
                try:
                    self.source_rate = self.xp.asarray(source_rate, device=self.device, dtype=self.xp.float32)
                except TypeError:
                    self.source_rate = self.xp.asarray(source_rate, dtype=self.xp.float32)
            else:
                try:
                    self.source_rate = self.xp.full((self.grid.nx, self.grid.ny), float(source_rate), device=self.device, dtype=self.xp.float32)
                except TypeError:
                    self.source_rate = self.xp.full((self.grid.nx, self.grid.ny), float(source_rate), dtype=self.xp.float32)
        if roughness_n is not None:
            try:
                self.manning_n_field = self.xp.asarray(roughness_n, device=self.device, dtype=self.xp.float32)
            except TypeError:
                self.manning_n_field = self.xp.asarray(roughness_n, dtype=self.xp.float32)
        if overflow_mask is not None:
            # ensure binary mask
            try:
                om = self.xp.asarray(overflow_mask, device=self.device)
            except TypeError:
                om = self.xp.asarray(overflow_mask)
            if hasattr(self.xp, 'where'):
                om = self.xp.where(om > 0, 1.0, 0.0)
            # Cast to float32 for computations (NumPy or Torch)
            if hasattr(om, 'astype'):
                self.overflow_mask = om.astype(self.xp.float32)
            else:
                self.overflow_mask = om.to(self.xp.float32)
        if crest_elev is not None:
            try:
                self.crest_elev = self.xp.asarray(crest_elev, device=self.device, dtype=self.xp.float32)
            except TypeError:
                self.crest_elev = self.xp.asarray(crest_elev, dtype=self.xp.float32)
    ...
    def total_mass(self) -> float:
        # Compute mass excluding the numerical film (h_min), to avoid artificial mass from positivity clamp
        try:
            h_eff = self.h - self.xp.asarray(self.prm.h_min, device=self.device)
        except TypeError:
            h_eff = self.h - self.xp.asarray(self.prm.h_min)
        if hasattr(self.xp, 'maximum'):
            h_eff = self.xp.maximum(h_eff, self.xp.asarray(0.0, device=self.device) if hasattr(self.h, 'device') else self.xp.asarray(0.0))
        else:
            h_eff = np.maximum(h_eff, 0.0)
        return float(self.xp.sum(h_eff) * self.grid.dx * self.grid.dy)
    ...
    def apply_sponge(self, h_target: Optional[np.ndarray] = None, dt: float = 0.0):
        ...
        # Left/right
        lam = dt / tau
        # Track boundary flux as the relaxation-induced change at the edges
        cell_area = self.grid.dx * self.grid.dy
        # West edge
        h_old = self.h[:w, :].copy()
        self.h[:w, :] = (1 - lam) * self.h[:w, :] + lam * h_target[:w, :]
        try:
            dV = float(self.xp.sum(self.h[:w, :] - h_old)) * cell_area
        except Exception:
            dV = float(np.sum(to_numpy(self.h[:w, :] - h_old))) * cell_area
        self.boundary_flux_total += dV
        # East edge
        h_old = self.h[-w:, :].copy()
        self.h[-w:, :] = (1 - lam) * self.h[-w:, :] + lam * h_target[-w:, :]
        try:
    ...
    def rhs(self, h, u, v) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        try:
            ...
            h_min_tensor = xp.asarray(self.prm.h_min, dtype=xp.float32)
            g_tensor = xp.asarray(g, dtype=xp.float32)
            zero_tensor = xp.asarray(0.0, dtype=xp.float32)
            one_tensor = xp.asarray(1.0, dtype=xp.float32)
            eps_tensor = xp.asarray(1e-12, dtype=xp.float32)
        ...
        if self.mode == "dw":
            ...
            deta_dx = (xp.roll(eta, -1, 0) - xp.roll(eta, 1, 0)) / (2.0 * grid.dx)
            deta_dy = (xp.roll(eta, -1, 1) - xp.roll(eta, 1, 1)) / (2.0 * grid.dy)
            ...
            if self.infil_rate is not None:
                # Cap infiltration by available water this step (avoid overdraw)
                dt_loc = getattr(self, '_dt', 0.0)
                if dt_loc > 0.0:
                    try:
                        h_min_tensor = xp.asarray(self.prm.h_min, device=self.device, dtype=xp.float32)
                    except TypeError:
                        h_min_tensor = xp.asarray(self.prm.h_min, dtype=xp.float32)
                    avail = xp.maximum(h - h_min_tensor, 0.0) / dt_loc
                    eff_infil = xp.minimum(self.infil_rate, avail)
                    dhdt = dhdt - eff_infil
                else:
                    dhdt = dhdt - self.infil_rate
            if getattr(self, 'source_rate', None) is not None:
                dhdt = dhdt + self.source_rate
            ...
            # Accumulate boundary flux (east and north edges treated as open)
            try:
                if dt > 0.0:
                    Q_east = float(xp.sum(q_e[nx-2, :])) * dy  if nx > 1 else 0.0
                    Q_north = float(xp.sum(q_s[:, ny-2])) * dx if ny > 1 else 0.0
                    # Positive here means out of domain; define boundary_flux_total positive as inflow
                    self.boundary_flux_total += dt * (0.0 - (Q_east + Q_north))
            except Exception:
                pass
    ...
    def apply_structures(self, dt: float):
        ...
        dx, dy = self.grid.dx, self.grid.dy
        cell_area = dx * dy
        # Bankfull overtop (canal/drain/barrier overtopping): broad-crested weir per cell to neighbors
        if self.overflow_mask is not None and self.crest_elev is not None:
            Cd = float(self.overflow_Cd)
            mask = self.overflow_mask
            # Use water surface elevation eta = h + bed; excess head above crest
            eta_loc = self.h + (self.bed if self.bed is not None else 0.0)
            Hx = eta_loc - self.crest_elev
            ...
            if float(self.xp.max(Hx)) > 0.0:
                ...
                q_w = self.xp.roll(q_e, 1, 0)
                # South/north
                q_s = Cd * W_s * (Hx ** 1.5) * math.sqrt(2 * g)
                q_n = self.xp.roll(q_s, 1, 1)
                # Only from overflow cells to non-overflow neighbors
                nei_e = 1.0 - self.xp.roll(mask, -1, 0)
                nei_w = 1.0 - self.xp.roll(mask,  1, 0)
                nei_s = 1.0 - self.xp.roll(mask, -1, 1)
                nei_n = 1.0 - self.xp.roll(mask,  1, 1)
                q_e = q_e * mask * nei_e
                q_w = q_w * self.xp.roll(mask, -1, axis=0) * nei_w  # from east neighbor into this
                q_s = q_s * mask * nei_s
                q_n = q_n * self.xp.roll(mask, -1, axis=1) * nei_n
    ...
    def choose_dt(self) -> float:
        ...
        if self.mode in ("dw", "dw_fv"):
            ...
            try:
                ...
                H = xp.maximum(self.h, xp.asarray(self.prm.h_min))
            Z = self.bed if self.bed is not None else 0.0
            eta = H + Z
            dx, dy = self.grid.dx, self.grid.dy
            cell_area = dx * dy
            # Slopes to neighbors (only downslope outflows considered)
            eta_e = xp.roll(eta, -1, 0)
            eta_w = xp.roll(eta, 1, 0)
            eta_n = xp.roll(eta, 1, 1)
            eta_s = xp.roll(eta, -1, 1)
            ...
            H_e = xp.minimum(H, xp.roll(H, -1, 0))
            H_w = xp.minimum(H, xp.roll(H, 1, 0))
            H_n = xp.minimum(H, xp.roll(H, 1, 1))
            H_s = xp.minimum(H, xp.roll(H, -1, 1))
            try:
                K = xp.sqrt(xp.asarray(g, device=self.device)) / n
            except TypeError:
                K = xp.sqrt(xp.asarray(g)) / n
    ...
    def run(self, t_end: float, output_every: float = 0.0, verbose: bool = True,
        ...
        while t < t_end:
            ...
            logs["dt"].append((t, dt))
            # Accumulate mass budget ledgers
            cell_area = self.grid.dx * self.grid.dy
            if self.rain_rate is not None:
                try:
                    self.rain_total += float(self.xp.sum(self.rain_rate)) * cell_area * dt
                except Exception:
                    self.rain_total += float(np.sum(to_numpy(self.rain_rate))) * cell_area * dt
            if self.infil_rate is not None:
                try:
                    self.infil_total += float(self.xp.sum(self.infil_rate)) * cell_area * dt
                except Exception:
                    self.infil_total += float(np.sum(to_numpy(self.infil_rate))) * cell_area * dt
            if getattr(self, 'source_rate', None) is not None:
                try:
                    self.source_total += float(self.xp.sum(self.source_rate)) * cell_area * dt
                except Exception:
                    self.source_total += float(np.sum(to_numpy(self.source_rate))) * cell_area * dt
            if getattr(self, 'pond_storage_rate', None) is not None:
                try:
                    self.pond_storage_total += float(self.xp.sum(self.pond_storage_rate)) * cell_area * dt
                except Exception:
                    self.pond_storage_total += float(np.sum(to_numpy(self.pond_storage_rate))) * cell_area * dt
            # Output hook (placeholder — users can insert writers here)
            if output_every > 0.0 and t >= next_out:
                if verbose:
                    m_change_pct = 100.0 * (mass - self.mass0) / max(1e-12, self.mass0)
                    # Diagnostics: Froude and slope
                    xp = self.xp
                    c = xp.sqrt(self.prm.g * self.h)
                    spd = xp.sqrt(self.u*self.u + self.v*self.v)
                    fr_max = float(xp.max(spd / xp.maximum(c, xp.asarray(1e-9, device=self.device))))
                    eta = self.h + (self.bed if self.bed is not None else 0.0)
                    # Use xp.roll signature compatible with both numpy and cupy/torch via positional args
                    deta_dx = (xp.roll(eta, -1, 0) - xp.roll(eta, 1, 0)) / (2.0 * self.grid.dx)
                    deta_dy = (xp.roll(eta, -1, 1) - xp.roll(eta, 1, 1)) / (2.0 * self.grid.dy)
                    slope_max = float(xp.max(xp.sqrt(deta_dx*deta_dx + deta_dy*deta_dy)))
                    print(f"t={t:8.2f}s  dt={dt:7.4f}s  vmax={vmax:7.3f}  Fr_max={fr_max:5.3f}  slope_max={slope_max:5.3f}  mass_chg={m_change_pct:7.2f}%")
